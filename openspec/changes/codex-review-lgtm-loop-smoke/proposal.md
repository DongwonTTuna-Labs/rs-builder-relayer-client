...
plans, stage05-stage08 autofix plumbing, an AI semantic patch safety gate over the
exact merged patch hash, and stage09 issue fallback. This change creates a
repository-native OpenSpec artifact that can be used to verify the whole PR feedback
...
prepare an autofix patch, semantically approve the exact patch hash, and push the fix
through the GitHub App token path without a manual enable variable.
...
- Require Stage06 semantic safety to approve the exact docs patch hash before Stage07
  can validate or push it.
