...
def main() -> int:
    ...
    bootstrap_scripts_ref = os.environ.get("CODEX_BOOTSTRAP_SCRIPTS_REF", "").strip()
    if os.environ.get("GITHUB_EVENT_NAME") == "workflow_dispatch" and bootstrap_scripts_ref:
        set_output("scripts_ref", bootstrap_scripts_ref)
    else:
