...
: "${CODEX_LB_API_KEY:?CODEX_LB_API_KEY is required}"
...
codex --enable use_legacy_landlock --ask-for-approval never exec \
  ...
  -c 'model_providers.codex-lb.base_url="https://open-ai.dongwontuna.net/backend-api/codex"' \
  ...
  -c 'shell_environment_policy.exclude=["CODEX_LB_API_KEY"]' \
