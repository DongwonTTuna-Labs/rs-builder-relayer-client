...
def test_collect_agent_results_prefers_validated_result_over_raw_duplicate(tmp_path):
    manifest = {"tasks": [{"task_id":"t1"}]}
    task_dir = tmp_path / "codex-review-stage05-agent-t1"
    task_dir.mkdir()
    raw = task_dir / "result.json"
    validated = task_dir / "result.validated.json"
    raw.write_text(json.dumps({"schema_version":"stage05-fix-agent-result.v1", "task_id":"t1", "status":"no_safe_fix"}), encoding="utf-8")
    validated.write_text(json.dumps({"schema_version":"stage05-fix-agent-result.v1", "task_id":"t1", "status":"patched", "patch":"diff --git a/docs/x b/docs/x\n"}), encoding="utf-8")
...
def test_collect_agent_results_keeps_raw_only_results_when_no_validated_result(tmp_path):
    manifest = {"tasks": [{"task_id":"t1"}, {"task_id":"t2"}]}
    t1 = tmp_path / "codex-review-stage05-agent-t1"
    t2 = tmp_path / "codex-review-stage05-agent-t2"
    t1.mkdir()
    t2.mkdir()
    raw1 = t1 / "result.json"
    validated1 = t1 / "result.validated.json"
    raw2 = t2 / "result.json"
    raw1.write_text(json.dumps({"schema_version":"stage05-fix-agent-result.v1", "task_id":"t1", "status":"no_safe_fix"}), encoding="utf-8")
    validated1.write_text(json.dumps({"schema_version":"stage05-fix-agent-result.v1", "task_id":"t1", "status":"patched", "patch":"diff --git a/docs/x b/docs/x\n"}), encoding="utf-8")
    raw2.write_text(json.dumps({"schema_version":"stage05-fix-agent-result.v1", "task_id":"t2", "status":"no_safe_fix"}), encoding="utf-8")
...
def test_collect_agent_results_rejects_mixed_priority_duplicates_from_different_dirs(tmp_path):
    manifest = {"tasks": [{"task_id":"t1"}]}
    stale_dir = tmp_path / "codex-review-stage05-agent-t1-stale"
    current_dir = tmp_path / "codex-review-stage05-agent-t1-current"
    stale_dir.mkdir()
    current_dir.mkdir()
    stale_raw = stale_dir / "result.json"
    current_validated = current_dir / "result.validated.json"
    stale_raw.write_text(json.dumps({"schema_version":"stage05-fix-agent-result.v1", "task_id":"t1", "status":"no_safe_fix"}), encoding="utf-8")
    current_validated.write_text(json.dumps({"schema_version":"stage05-fix-agent-result.v1", "task_id":"t1", "status":"patched", "patch":"diff --git a/docs/x b/docs/x\n"}), encoding="utf-8")
