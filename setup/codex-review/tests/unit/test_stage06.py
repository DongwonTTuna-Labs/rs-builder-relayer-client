...
from codex_review.stages.stage06_fix_merge import (
...
class Stage06Tests(unittest.TestCase):
    ...
    def test_stage07_validation_commands_fall_back_to_diff_check_when_all_are_deferred(self):
        ...
        payload["tasks"][0]["test_plan"] = [
            "cargo test --workspace --all-features",
            "python3 -m unittest discover -s setup/codex-review/tests/workflow -p test_codex_review_orchestrator_workflow.py",
        ]
        outputs = fix_outputs()
        outputs["outputs"][0]["tests"] = [
            "cargo clippy --workspace --all-targets --all-features -- -D warnings",
    ...
    def test_deferred_validation_command_argv_allows_exact_pr_head_commands(self):
        cases = {
            ...
            "python3 -m unittest discover -s setup/codex-review/tests/workflow": [
                ...
                "discover",
                "-s",
                "setup/codex-review/tests/workflow",
            ],
        }
    ...
    def test_workflow_file_changes_are_allowed_when_task_allows_them(self):
        payload = dispatch()
        payload["tasks"][0]["allowed_files"].append(".github/workflows/codex-review-orchestrator.yml")
        outputs = fix_outputs(touched_files=[".github/workflows/codex-review-orchestrator.yml"])
        outputs["outputs"][0]["patch"] = (
            "diff --git a/.github/workflows/codex-review-orchestrator.yml b/.github/workflows/codex-review-orchestrator.yml\n"
            "--- a/.github/workflows/codex-review-orchestrator.yml\n"
            "+++ b/.github/workflows/codex-review-orchestrator.yml\n"
            "@@ -1 +1 @@\n-old\n+new\n"
        )

        result = build_fix_merge_result(payload, outputs)

        self.assertEqual([".github/workflows/codex-review-orchestrator.yml"], result["touched_files"])
