...
def test_model_adapter_uses_fallback_without_command(monkeypatch, tmp_path):
    ...
    out = run_model_or_fallback(stage="stage01", prompt_path=None, output_path=tmp_path/"out.json", expected_schema="x.v1", fallback={"items": []})
...
def test_model_adapter_runs_command_and_reads_output(monkeypatch, tmp_path):
    ...
    out = run_model_or_fallback(stage="stage01", prompt_path=prompt, output_path=tmp_path/"out.json", expected_schema="stage01-axis-findings.v1", fallback={})
    assert out["ok"] is True
    assert out["schema_version"] == "stage01-axis-findings.v1"
...
def test_model_adapter_runs_from_trusted_cwd_and_exposes_target_repo(monkeypatch, tmp_path):
    ...
    out = run_model_or_fallback(
        ...
        expected_schema="stage05-fix-agent-result.v1",
        fallback={},
        cwd=trusted,
