...
def test_requires_human_review_blocks_non_openspec_approval():
...
def test_openspec_backed_generic_requires_human_review_flag_is_not_a_stop():
    ...
    out=validate_chief_decision({"status":"needs_human","reason":"generic review requested"}, plan, CFG)
    assert out["status"] == "approved_for_fix"


def test_openspec_backed_no_fix_or_rejected_plan_is_promoted_when_executable():
    plan={**PLAN,"openspec_backed":True,"acceptance_criteria":["OpenSpec task is satisfied"],"execution_blockers":[]}
    for status in ["no_fix_needed", "rejected_plan"]:
        out=validate_chief_decision({"status":status,"reason":"too conservative"}, plan, CFG)
        assert out["status"] == "approved_for_fix"
