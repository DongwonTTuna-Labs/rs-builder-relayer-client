...
def test_semantic_patch_safety_model_gates_push_validation():
    ...
    assert section.index("fix_merge validate-semantic-safety") < section.index("push validate-fix")
    assert "push write-validation-outputs" in section
...
def test_small_intra_workflow_handoffs_use_job_outputs_not_artifacts():
    ...
    for removed in [
        ...
        "codex-review-fix-merge-semantic-safety",
        "codex-issue-plan",
    ]:
