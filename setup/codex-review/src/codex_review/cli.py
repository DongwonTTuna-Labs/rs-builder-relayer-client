...
from .paths import prompt_path, schema_path
from .security.relay import default_relay_contract, normalize_codex_args
...
def run_stage(stage: str, args: argparse.Namespace) -> int:
    if stage == "stage00-resolve-gate" and getattr(args, "inventory", None):
        ...
        from .stages.stage00_resolve_gate import build_resolve_gate_result
    ...
    if stage == "stage01-review" and getattr(args, "request", None) and getattr(args, "model_output", None):
        from .artifacts import read_json_artifact
        from .stages.stage01_review import build_review_result
    ...
    if stage == "stage02-techlead" and getattr(args, "review", None):
        from .artifacts import read_json_artifact
        from .stages.stage02_techlead import build_techlead_result
    ...
    if (
        ...
        from .artifacts import read_json_artifact
        from .stages.stage03_design import build_design_result
    ...
    if stage == "stage04-design-chief" and getattr(args, "design", None) and getattr(args, "model_approval", None):
        from .artifacts import read_json_artifact
        from .stages.stage04_design_chief import build_design_chief_result
    ...
    if stage == "stage05-fix-dispatch" and getattr(args, "design", None) and getattr(args, "design_chief", None):
        from .artifacts import read_json_artifact
        from .stages.stage05_fix_dispatch import build_fix_dispatch_result
    ...
    if stage == "stage06-fix-merge" and getattr(args, "dispatch", None) and getattr(args, "fix_outputs", None):
        from .artifacts import read_json_artifact
        from .stages.stage06_fix_merge import build_fix_merge_result
    ...
    if stage == "stage07-push" and getattr(args, "fix_merge", None) and getattr(args, "trusted_push", None):
        from .artifacts import read_json_artifact
        from .stages.stage07_push import build_push_result
    ...
    if stage == "stage08-reentry" and getattr(args, "push", None) and getattr(args, "run_state", None):
        from .artifacts import read_json_artifact
        from .stages.stage08_reentry import build_reentry_result
...
def run_stage00_context(args: argparse.Namespace) -> int:
    from .artifacts import read_json_artifact
    from .stages.stage00_resolve_gate import build_context_artifacts
...
def run_stage05_fallback_fix_outputs(args: argparse.Namespace) -> int:
    from .artifacts import read_json_artifact
    from .stages.stage06_fix_merge import build_conflict_fix_outputs
...
def run_stage06_deferred_validation(args: argparse.Namespace) -> int:
    from .artifacts import read_json_artifact
    from .stages.stage06_fix_merge import run_deferred_validation
...
def run_stage06_finalize_validation(args: argparse.Namespace) -> int:
    from .artifacts import read_json_artifact
    from .stages.stage06_fix_merge import build_validated_fix_merge_result
...
def run_stage00_lifecycle(args: argparse.Namespace) -> int:
    from .artifacts import read_json_artifact
    from .stages.stage00_resolve_gate import build_lifecycle_result
...
def run_stage02_comment(args: argparse.Namespace) -> int:
    from .artifacts import read_json_artifact
    from .stages.stage02_techlead import build_review_comment_body
...
def run_stage07_validation(args: argparse.Namespace) -> int:
    from .stages.stage07_push import run_validation_commands
...
def run_print_schema(args: argparse.Namespace) -> int:
    path = schema_path(args.name)
    if not path.is_file():
...
def run_render_prompt(args: argparse.Namespace) -> int:
    path = prompt_path(args.stage)
    if not path.is_file():
        print(f"prompt not found: {path}", file=sys.stderr)
        return 1
    parts = [path.read_text(encoding="utf-8").rstrip(), ""]
    for append_path in args.append:
        parts.append(append_path.read_text(encoding="utf-8").rstrip())
        parts.append("")
    rendered = "\n".join(parts)
    if args.out:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    schema.add_argument("--name", required=True)
    schema.set_defaults(func=run_print_schema)
    prompt = subparsers.add_parser("render-prompt")
    prompt.add_argument("--stage", required=True)
    prompt.add_argument("--out", type=Path, required=False)
    prompt.add_argument("--append", type=Path, action="append", default=[])
    prompt.set_defaults(func=run_render_prompt)
