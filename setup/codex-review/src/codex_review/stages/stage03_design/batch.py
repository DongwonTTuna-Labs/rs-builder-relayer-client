...
def make_cluster_batches(clusters: dict[str, Any], config: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [{"schema_version":"stage03-cluster-batch.v1","batch_index":i//size,"clusters":items[i:i+size]} for i in range(0,len(items),size)] or [{"schema_version":"stage03-cluster-batch.v1","batch_index":0,"clusters":[]}]


def write_cluster_batch(batch: dict[str, Any], out_path: str | Path) -> Path:
    return write_json(out_path, batch, "stage03-cluster-batch.v1")
