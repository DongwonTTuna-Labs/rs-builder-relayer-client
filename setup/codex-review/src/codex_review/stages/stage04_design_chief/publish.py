...
def _verify_design_publish_allowed(pr_context: dict[str, Any] | None, token: str | None, dry_run: bool) -> None:
    if dry_run:
        return
    if not pr_context:
        raise ValidationError("stage04 publish requires pr_context")
    missing = [key for key in ["owner", "repo", "pr_number"] if not pr_context.get(key)]
    if missing:
        raise ValidationError(f"stage04 publish requires pr_context fields: {', '.join(missing)}")
    assert_installation_token_for_repo(token, pr_context["owner"], pr_context["repo"], permissions_for_write_mode("stage04"))
    ...
    if expected and current and expected != current:
        raise ValidationError(f"head SHA drift before stage04 publish: expected {expected}, current {current}")
...
def publish_design_summary(plan: dict[str, Any], chief_decision: dict[str, Any], token: str | None = None, pr_context: dict[str, Any] | None = None, dry_run: bool = False) -> dict[str, Any]:
    ...
    report = {"schema_version": "stage04-publish-report.v1", "dry_run": dry_run, "body": body}
...
def write_design_publish_report(report: dict[str, Any], out_path: str | Path) -> Path:
    return write_json(out_path, report, "stage04-publish-report.v1")
