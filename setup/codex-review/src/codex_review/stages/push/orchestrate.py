...
def _no_diff_repeat_result(merged_fix: dict[str, Any], patch: str, head: str | None, reason: str, *, schema_version: str = "push-validated-fix.v1") -> dict[str, Any]:
    ...
    *,
    ...
    try:
        try:
            ...
            if _patch_already_applied(patch, repo_path):
                return {**_no_diff_repeat_result(merged_fix, patch, old_head, f"patch is already present on the PR head: {exc}", schema_version="push-result.v1"), "validation_result": validation_result}
            raise
        try:
            run_diff_check(repo_path)
        except ValidationError as exc:
            return {**_no_diff_repeat_result(merged_fix, patch, old_head, str(exc), schema_version="push-result.v1"), "validation_result": validation_result}
        applied_diff = collect_applied_diff(repo_path)
