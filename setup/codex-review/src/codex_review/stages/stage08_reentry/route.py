...
def determine_reentry_expectation(push_result: dict[str, Any]) -> dict[str, Any]:
    ...
    return {"schema_version":"stage08-route.v1","expect_synchronize":pushed,"next_entry":"stage00" if pushed else "none"}

def write_reentry_outputs(record: dict[str, Any]) -> None:
    write_output("expect_synchronize", record.get("expect_synchronize") or record.get("next_entry") == "stage00_on_synchronize")
