...
def include_no_safe_fix_contract(prompt: str) -> str:
    return prompt + "\nFor OpenSpec-backed tasks, do not use no_safe_fix for uncertainty or conservatism. Use no_safe_fix only for a concrete mechanical blocker such as allowed_files impossibility, missing source file context that prevents a patch, or a policy conflict that stage09 must track."
