...
def persist_reentry_loop_state(record: dict[str, Any], pr_context: dict[str, Any], token: str | None) -> dict[str, Any]:
    ...
    if missing:
        raise ValidationError(f"reentry loop-state persistence requires pr_context fields: {', '.join(missing)}")
    assert_installation_token_for_repo(token, pr_context["owner"], pr_context["repo"], permissions_for_write_mode("reentry"))
