...
def _verify_publish_allowed(pr_context: dict[str, Any], token: str | None, dry_run: bool) -> str | None:
    ...
    if missing:
        raise ValidationError(f"stage02 publish requires pr_context fields: {', '.join(missing)}")
    assert_installation_token_for_repo(token, pr_context["owner"], pr_context["repo"], permissions_for_write_mode("stage02"))
    ...
    if expected and current and expected != current:
        raise ValidationError(f"head SHA drift before stage02 publish: expected {expected}, current {current}")
...
def publish_review(publication: dict[str, Any], pr_context: dict[str, Any], changed_line_map: dict[str, Any], token: str | None, config: dict[str, Any], dry_run: bool = False) -> dict[str, Any]:
    ...
    report={"schema_version":"stage02-publish-report.v1","inline_comment_count":len(comments),"dry_run": dry_run,"comments":comments,"body":body,"commit_id": commit_id}
...
def write_publish_report(report: dict[str, Any], out_path: str | Path) -> Path:
    return write_json(out_path, report, "stage02-publish-report.v1")
