...
def build_review_publication(decision: dict[str, Any], combined_findings: dict[str, Any], config: dict[str, Any]) -> dict[str, Any]:
    ...
    return {"schema_version":"stage02-review-publication.v1","status":determine_techlead_status(decision),"needs_design":determine_needs_design(decision),"inline_comments":inline,"summary_items":summary,"deferred_items":deferred,"all_decisions":decision.get("decisions", [])}


def write_review_publication(publication: dict[str, Any], out_path: str | Path) -> Path:
    return write_json(out_path, publication, "stage02-review-publication.v1")
