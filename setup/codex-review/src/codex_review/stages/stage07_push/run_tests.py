...
def run_required_tests(commands: list[str], repo_path: str | Path) -> dict[str, Any]:
    ...
    return {"schema_version": "stage07-test-report.v1", "results": results, "passed": all(r["passed"] for r in results)}


def write_test_report(report: dict[str, Any], out_path: str | Path) -> Path:
    return write_json(out_path, report, "stage07-test-report.v1")
