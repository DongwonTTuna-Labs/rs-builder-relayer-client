...
def build_premerge_report(results: dict[str, Any]) -> dict[str, Any]:
    return {"schema_version":"stage06-premerge-report.v1","clean":bool(results.get("clean")),"results":results.get("results", []),"temp_worktree":results.get("temp_worktree"),"error":results.get("error")}
...
def write_premerge_report(report: dict[str, Any], out_path: str | Path) -> Path:
    return write_json(out_path, report, "stage06-premerge-report.v1")
...
def create_merged_fix_from_premerge(premerge_report: dict[str, Any], collection: dict[str, Any], pr_context: dict[str, Any] | None = None, out_path: str | Path | None = None) -> dict[str, Any]:
    """Create a stage06 merged-fix artifact from clean deterministic premerge results.
    ...
    if not premerge_report.get("clean"):
        return {"schema_version": "stage06-merged-fix.v1", "status": "blocked", "patch": "", "expected_head_sha": (pr_context or {}).get("head_sha"), "premerge_clean": False, "conflicts": premerge_report.get("results", [])}
