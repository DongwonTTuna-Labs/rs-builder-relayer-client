...
def collect_thread_inventory(pr_context: dict[str, Any], threads: list[dict[str, Any]], comments: list[dict[str, Any]] | None, config: dict[str, Any]) -> dict[str, Any]:
    ...
    return {"schema_version": "stage00-thread-inventory.v1", "head_sha": pr_context.get("head_sha"), "pr_number": pr_context.get("pr_number"), "items": items, "root_cause_summary": aggregate_root_cause_metadata(items), "thread_count": len(items)}
