...
def build_gate_result(inventory: dict[str, Any], lifecycle_result: dict[str, Any], apply_report: dict[str, Any] | None, loop_state: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    return {"schema_version": "stage00-gate-result.v1", "route": route, "head_sha": inventory.get("head_sha"), "thread_count": len(inventory.get("items", [])), "decisions": decisions, "apply_report": apply_report or {}}


def write_gate_result(gate_result: dict[str, Any], out_path: str | Path) -> Path:
    return write_json(out_path, gate_result, "stage00-gate-result.v1")
