...
def validate_lifecycle_result(result: dict[str, Any], inventory: dict[str, Any]) -> dict[str, Any]:
    if result.get("schema_version") and result.get("schema_version") != "stage00-lifecycle-result.v1":
    ...
    out=dict(result); out["schema_version"]="stage00-lifecycle-result.v1"; out["decisions"]=decisions
    return out


def write_validated_lifecycle_result(result: dict[str, Any], out_path: str | Path) -> Path:
    return write_json(out_path, result, "stage00-lifecycle-result.v1")
