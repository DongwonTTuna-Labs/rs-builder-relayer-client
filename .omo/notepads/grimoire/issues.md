...
- ROOT CAUSE (who/what/when/why/how): who=`.github/workflows/grimoire-attune.yml` and `.github/workflows/grimoire.yml` step `env:` mapping; what=the relay key was mapped directly from the GitHub secret named `AI_RELAY_API_KEY` at 4 sites (attune 58,191; grimoire 242,269); when=discovered while planning live setup after user confirmed runner is online and the relay key is injected via self-hosted runner (local docker) env, not a GitHub Actions secret; why=when the secret is unset the expression resolves to an empty string and shadows the runner-inherited `AI_RELAY_API_KEY` within the step, so the smoke/driver fails closed even though the key exists in the runner env; how=mirror the `CODEX_LOOP_PAT` pattern by mapping the secret to `AI_RELAY_API_KEY_SECRET`, resolving the selected value from `AI_RELAY_API_KEY_SECRET` first and runner `AI_RELAY_API_KEY` second, masking that selected value, then exporting the selected value for opencode child inheritance.
