...
fn trusted_production_permit_token_for_action(
    ...
    let evidence = DepositWalletOwnerSerializationEvidence::new(
        ...
        "unit-test trusted production owner serialization guard",
        format!("unit-test-production-owner-lease-{owner:?}-{action:?}"),
        1_699_999_900,
        1_700_000_200,
    )
    .unwrap();
    DepositWalletMutationPermit {
        owner,
        reason: "trusted unit-test production capability".to_string(),
...
async fn sign_and_submit_batch_with_lease(
    client: &DepositWalletRelayerClient,
...
fn record_trusted_terminal_observation(
    client: &DepositWalletRelayerClient,
    owner: Address,
    transaction_id: &str,
    ...
    let mut mutation_state = client.mutation_state_for_owner(owner).unwrap();
    mutation_state.terminal_observations.insert(
        transaction_id.to_string(),
        OwnerTransactionTerminalObservation {
            observed_state,
            transaction_hash: transaction_hash.map(ToOwned::to_owned),
        },
    );
}
...
#[tokio::test]
async fn get_wallet_nonce_with_lease_rejects_production_before_http() {
    let owner = address(WALLET_OWNER);
    let url = DepositWalletRelayerUrl::parse("https://relayer-v2.polymarket.com").unwrap();
    let client = test_client(url);
    let permit = trusted_production_permit_token_for_action(
        &client,
        owner,
    ...
    assert!(
        error
            .to_string()
            .contains("production WALLET nonce lease reads are disabled")
    );
}
...
async fn public_mutation_apis_default_deny_before_http() {
    ...
    let lease = client
        ...
        .await
        .unwrap();
    let error = sign_and_submit_batch_with_lease(
        &client,
...
async fn sign_and_submit_wallet_batch_with_nonce_lease_sends_fixture_body() {
    ...
    let lease = client
        ...
        .await
        .unwrap();
    let expected_deposit_wallet = signed.deposit_wallet();
    let expected_chain_id = signed.chain_id();
    let expected_nonce = signed.nonce();
    let receipt = client
        .sign_and_submit_wallet_batch_with_nonce_lease(lease, wallet_batch_permit_for(owner), |ctx| {
            assert_eq!(ctx.owner(), owner);
            assert_eq!(ctx.nonce_owner(), owner);
            assert_eq!(ctx.submit_from(), owner);
...
async fn signed_wallet_batch_deadline_expiry_blocks_submit_before_post() {
    ...
    let lease = client
        ...
        .await
        .unwrap();
    let error = sign_and_submit_batch_with_lease(&client, signed, batch_permit, lease)
        .await
        .unwrap_err();
...
#[test]
fn manual_reconciliation_rejects_mismatched_terminal_observation_without_partial_delete() {
    let owner = address(WALLET_OWNER);
    let payload_hash =
        "0x9123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdea".to_string();
    let client = test_client(DepositWalletRelayerUrl::loopback("http://127.0.0.1/").unwrap());
    ...
    client
        ...
        .unwrap();
    record_trusted_terminal_observation(
        &client,
        owner,
        "tx-ambiguous",
        RelayerTransactionState::Failed,
...
#[tokio::test]
async fn terminal_owner_poll_releases_inflight_owner_block_before_resubmit() {
    let owner = address(WALLET_OWNER);
    ...
    assert!(terminal.transaction_hash.is_some());
    assert!(client.ambiguous_submit_block(owner).is_none());
...
#[tokio::test]
async fn terminal_failure_poll_releases_owner_block_before_resubmit() {
    let owner = address(WALLET_OWNER);
    let (url, handle) = spawn_server(vec![
        TestResponse::json(
            "200 OK",
...
#[tokio::test]
async fn pending_owner_poll_keeps_submit_block_before_resubmit() {
    let owner = address(WALLET_OWNER);
    let mut pending = wallet_create_transaction_response_value("tx-pending-inflight", "STATE_MINED");
    pending.as_object_mut().unwrap().remove("transactionHash");
    let (url, handle) = spawn_server(vec![
        TestResponse::json(
            "200 OK",
    ...
    let first = client
        ...
        .await
        .unwrap();
    assert_eq!(first.transaction_id, "tx-pending-inflight");
    let pending = client
        .get_transaction_for_owner(owner, "tx-pending-inflight")
        .await
        .unwrap_err();
    assert!(pending.is_deposit_wallet_transaction_absent());
    let blocked = client
        .submit_wallet_create(owner, wallet_create_permit_for(owner))
        .await
        .unwrap_err();
...
async fn reconciliation_required_poll_transitions_inflight_to_ambiguous_block() {
    ...
    let mut confirmed_empty_hash =
        wallet_create_transaction_response_value("tx-reconcile-poll-confirmed-empty-hash", "STATE_CONFIRMED");
    confirmed_empty_hash["transactionHash"] = json!("");
    ...
    for (transaction_id, transaction_response) in [
        ...
        (
            "tx-reconcile-poll-confirmed-empty-hash",
...
#[test]
fn manual_reconciliation_clears_multiple_transactions_one_at_a_time() {
    let owner = address(WALLET_OWNER);
    let payload_hash =
    ...
    client
        ...
        .unwrap();
    record_trusted_terminal_observation(&client, owner, "tx-a", RelayerTransactionState::Failed, None);
    let first_evidence = manual_reconciliation_evidence(
        owner,
        payload_hash.clone(),
        "tx-a",
        RelayerTransactionState::Failed,
    ...
    record_trusted_terminal_observation(&client, owner, "tx-b", RelayerTransactionState::Failed, None);
    let second_evidence = manual_reconciliation_evidence(
        owner,
        payload_hash,
        "tx-b",
        RelayerTransactionState::Failed,
...
fn mutation_permit_rejects_owner_scope_and_freshness_failures() {
    ...
    for (label, acquired_at, expires_at) in [
        ("acquired-now", 1_700_000_000, 1_700_000_001),
        (
            "expires-next-second",
            1_699_999_999,
            1_700_000_001,
        ...
        (
            "max-valid-lease-duration",
            1_700_000_001 - MAX_OWNER_SERIALIZATION_LEASE_SECONDS,
            1_700_000_001,
        ),
    ] {
        let evidence = DepositWalletOwnerSerializationEvidence::new(
        ...
        .unwrap();
        let permit =
            DepositWalletMutationPermit::from_owner_serialization_evidence(label, evidence)
                .unwrap();
        client
            .ensure_permit_token_for_action(
                &permit,
    ...
    let expires_now = DepositWalletOwnerSerializationEvidence::new(
        owner,
        mutation_scope(DepositWalletMutationAction::WalletCreate),
        "unit-test owner serialization guard",
        "expires-now-lease",
    ...
    .unwrap();
    let expires_now_permit =
        DepositWalletMutationPermit::from_owner_serialization_evidence("expires-now", expires_now)
            .unwrap();
    let expires_now = client
        .ensure_permit_token_for_action(
            &expires_now_permit,
            owner,
            DepositWalletMutationAction::WalletCreate,
        ...
        .unwrap_err();
    assert!(expires_now.is_deposit_wallet_mutation_blocked());
    assert!(expires_now.to_string().contains("expired"));
...
#[tokio::test]
async fn partial_submit_response_alias_is_not_trusted_transaction_owner() {
    let owner = address(WALLET_OWNER);
    let (url, handle) = spawn_server(vec![TestResponse::json(
        "200 OK",
...
#[tokio::test]
async fn idless_manual_reconciliation_rejects_unrecorded_transaction_id_owner_record_conflict() {
    let owner = address(WALLET_OWNER);
    ...
    assert_eq!(
        client.ambiguous_submit_transaction_ids(owner),
        vec!["tx-record-conflict".to_string()]
    );
    let evidence = idless_reconciliation_evidence(owner, payload_hash.clone(), 1_700_000_001);
    ...
    assert!(error.is_deposit_wallet_reconciliation_required());
    assert!(
        error.to_string().contains("observed transaction ids"),
        "{error}"
    );
...
#[tokio::test]
async fn malformed_id_submit_records_unrecorded_transaction_id_after_record_failure() {
    let owner = address(WALLET_OWNER);
    let conflicting_payload_hash =
        "0x9123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef".to_string();
    let (url, handle) = spawn_server(vec![TestResponse::json(
        "200 OK",
        json!({"transactionID": "tx-malformed-record-conflict", "state": 7}).to_string(),
    )])
    .await;
    let client = test_client(url);
    client
        .record_transaction_owner(
            "tx-malformed-record-conflict",
            owner,
            conflicting_payload_hash,
            WALLET_CREATE_TRANSACTION_TYPE,
        )
    ...
    let payload_hash = client
        .ambiguous_submit_block(owner)
        .expect("malformed id response should leave owner blocked");
    assert_eq!(
        client.ambiguous_submit_transaction_ids(owner),
        vec!["tx-malformed-record-conflict".to_string()]
    );
    let blocked = client
        .submit_wallet_create(owner, wallet_create_permit_for(owner))
        .await
...
#[tokio::test]
async fn idless_manual_reconciliation_rejects_unrecorded_new_transaction_id_record_failure() {
    let owner = address(WALLET_OWNER);
    let conflicting_payload_hash =
        "0x8123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef".to_string();
    let (url, handle) = spawn_server(vec![TestResponse::json(
    ...
    assert_eq!(
        client.ambiguous_submit_transaction_ids(owner),
        vec!["tx-new-record-conflict".to_string()]
    );
    let evidence = idless_reconciliation_evidence(owner, payload_hash.clone(), 1_700_000_001);
...
#[test]
fn transaction_id_manual_reconciliation_clears_unrecorded_transaction_id_observation() {
    let owner = address(WALLET_OWNER);
    let payload_hash =
    ...
    client.record_ambiguous(owner, payload_hash.clone()).unwrap();
    client
        .record_unrecorded_transaction_id_observation(owner, &payload_hash, "tx-unrecorded")
        .unwrap();
    assert_eq!(
        client.ambiguous_submit_transaction_ids(owner),
        vec!["tx-unrecorded".to_string()]
    );
    let idless_evidence = idless_reconciliation_evidence(owner, payload_hash.clone(), 1_700_000_001);
    ...
    assert!(error.is_deposit_wallet_reconciliation_required());
    assert!(error.to_string().contains("observed transaction ids"));

    let transaction_evidence = manual_reconciliation_evidence(
        owner,
        payload_hash.clone(),
        "tx-unrecorded",
        RelayerTransactionState::Failed,
...
#[test]
fn transaction_id_manual_reconciliation_clears_recorded_then_requires_unrecorded_transaction_id() {
    let owner = address(WALLET_OWNER);
    let payload_hash =
        "0x5123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef".to_string();
    let client = test_client(DepositWalletRelayerUrl::loopback("http://127.0.0.1/").unwrap());
    ...
    assert_eq!(
        client.ambiguous_submit_transaction_ids(owner),
        vec!["tx-recorded".to_string(), "tx-unrecorded".to_string()]
    );
    let evidence = manual_reconciliation_evidence(
        owner,
        payload_hash.clone(),
    ...
    let idless_error = client
        .clear_idless_ambiguous_submit_after_manual_reconciliation(
            idless_reconciliation_evidence(owner, payload_hash.clone(), 1_700_000_001),
            manual_reconciliation_permit_token_for(owner),
        )
        .unwrap_err();
    assert!(idless_error.is_deposit_wallet_reconciliation_required());
    assert!(idless_error.to_string().contains("observed transaction ids"));

    let unrecorded_evidence = manual_reconciliation_evidence(
        owner,
        payload_hash.clone(),
        "tx-unrecorded",
        RelayerTransactionState::Failed,
...
fn submit_response_parser_covers_abnormal_wire_shapes_and_terminal_states() {
    ...
    let malformed_id_with_state = json!({"transactionID": "tx-id-with-bad-state", "state": 7})
        .to_string();
    assert_eq!(
        extract_submit_transaction_id(malformed_id_with_state.as_bytes()),
        Some("tx-id-with-bad-state".to_string())
    );
    assert!(parse_submit_response(malformed_id_with_state.as_bytes()).is_err());
...
#[test]
fn transaction_response_treats_empty_transaction_hash_as_missing() {
    let transaction_id = "tx-empty-hash";
    let config = deposit_wallet_contract_config(137).unwrap();
    let mut response = transaction_response_value(transaction_id, "STATE_CONFIRMED");
    ...
    assert_eq!(parsed.receipt.state, RelayerTransactionState::Confirmed);
    assert_eq!(parsed.receipt.transaction_hash, None);
}
...
fn transaction_response_rejects_unproven_wire_evidence_boundaries() {
    ...
    let mut wallet_missing_to = transaction_response_value(target, "STATE_CONFIRMED");
    wallet_missing_to.as_object_mut().unwrap().remove("to");
    let mut wallet_to_mismatch = transaction_response_value(target, "STATE_CONFIRMED");
    wallet_to_mismatch["to"] = json!(other_address);
    let mut wallet_create_missing_to = wallet_create_transaction_response_value(target, "STATE_CONFIRMED");
    wallet_create_missing_to.as_object_mut().unwrap().remove("to");
    let mut wallet_create_to_mismatch = wallet_create_transaction_response_value(target, "STATE_CONFIRMED");
    wallet_create_to_mismatch["to"] = json!(other_address);
    ...
    for (label, response, expected_message) in [
        ...
        ("WALLET missing to", wallet_missing_to, "did not include to address"),
        (
            "WALLET to mismatch",
            wallet_to_mismatch,
            "did not match configured factory",
        ),
        (
            "WALLET-CREATE missing to",
            wallet_create_missing_to,
            "did not include to address",
        ),
        (
            "WALLET-CREATE to mismatch",
            wallet_create_to_mismatch,
            "did not match configured factory",
        ),
        (
